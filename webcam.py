...
pytesseract.pytesseract.tesseract_cmd = r"C:\Users\Raveesh MK\Anaconda3\Lib\site-packages\tesseract.exe"
        ...
        if classes[0][i] ==49:
            if scores[0][i] >= 0.5:
                ...
                if apx_distance <=0.5:
                    if mid_x > 0.3 and mid_x < 0.7:
                        cv2.putText(image_np, 'WARNING!!!', (50,50), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,0,255), 3)
                        print("Warning -Knife very close to the frame")
                        engine.say("Warning -Knife very close to the frame")
                        engine.runAndWait()
